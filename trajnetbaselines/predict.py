...
def predict(ground_truth, output_file):
    with trajnettools.show.paths(ground_truth, output_file) as ax:
        ...
        ax.plot([ground_truth[0][8].x] + [r.x for r in kf],
                [ground_truth[0][8].y] + [r.y for r in kf], color='orange', label='KF')
        ...
        ax.plot([ground_truth[0][8].x] + [r.x for r in lstmp],
                [ground_truth[0][8].y] + [r.y for r in lstmp], color='blue', label='LSTM')
        ...
        ax.plot([ground_truth[0][8].x] + [r.x for r in slstmp],
                [ground_truth[0][8].y] + [r.y for r in slstmp], color='red', label='Social LSTM')
...
def main():
    ...
    reader = trajnettools.Reader(args.dataset_file)
    for scene_id, ground_truth in reader.scenes(limit=5, as_paths=True):
