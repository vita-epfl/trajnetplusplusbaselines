...
def main(args):
    ...
    train_dset = list(trajnettools.load_all(args.path + '/train/**/*.ndjson'))
    # train_dset, train_loader = data_loader(args, train_path)
    logger.info("Initializing val dataset")
    val_dset = list(trajnettools.load_all(args.path + '/val/**/*.ndjson'))
    ...
    while t < args.num_iterations:
        ...
        for batch in loader(train_dset, args.batch_size):
            # print(batch)
            # pass
        ##get batch 
            if args.timing == 1:
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                metrics_val = check_accuracy(
                    args, val_dset, generator, discriminator, d_loss_fn
                )
                logger.info('Checking stats on train ...')
                metrics_train = check_accuracy(
                    args, train_dset, generator, discriminator,
...
def check_accuracy(
    args, loader_set, generator, discriminator, d_loss_fn, limit=False
):
    d_losses = []
    metrics = {}
    ...
    with torch.no_grad():
        # for batch in loader:
        for batch in loader(loader_set, args.batch_size):
            batch = [tensor.cuda() for tensor in batch]
...
def loader(dataset, batch_size=1):
    ...
    for ndx in range(0, l, batch_size):
        scenes = [torch.Tensor(scene.transpose(1, 0, 2)[~np.isnan(scene.transpose(1, 0, 2)).any(axis=1).any(axis=1)]).type(torch.float) for (_, scene) in dataset[ndx:min(ndx + batch_size, l)]]
        # scenes = [torch.Tensor(scene).type(torch.float) for (_, scene) in dataset[ndx:min(ndx + batch_size, l)]]
        # print(scenes)
        # import pdb
        # pdb.set_trace()
        ## Obs and Pred
