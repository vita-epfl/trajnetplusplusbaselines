"""Template for trainer of new model."""
...
from .model import MyNewModel, MyNewModelPredictor, drop_distant, generate_square_subsequent_mask
...
class Trainer(object):
    def __init__(self, model=None, criterion=None, optimizer=None, lr_scheduler=None,
        ...
        ###################### TODO ###############################
        # initialize trainer appropriately
        self.model = model if model is not None else MyNewModel()
    ...
    def loop(self, train_scenes, val_scenes, train_goals, val_goals, out, epochs=35, start_epoch=0):
        for epoch in range(start_epoch, epochs):
            if epoch % self.save_every == 0:
                ...
                MyNewModelPredictor(self.model).save(state, out + '.epoch{}'.format(epoch))
            self.train(train_scenes, train_goals, epoch)
            if self.val_flag and (epoch % 2 == 0):
                self.val(val_scenes, val_goals, epoch)
                # self.val(train_scenes, train_goals, epoch)


        state = {'epoch': epoch + 1, 'state_dict': self.model.state_dict(),
                 'optimizer': self.optimizer.state_dict(),
                 'scheduler': self.lr_scheduler.state_dict()}
        MyNewModelPredictor(self.model).save(state, out + '.epoch{}'.format(epoch + 1))
        MyNewModelPredictor(self.model).save(state, out)
    ...
    def val(self, scenes, goals, epoch):
        ...
        for scene_i, (filename, scene_id, paths) in enumerate(scenes):
            ...
            if ((scene_i + 1) % self.batch_size == 0) or ((scene_i + 1) == len(scenes)):
                ...
                ###################### TODO ###############################
                ## Get predictions 
                # (If Unimodal) predictions = (Num_peds x Num_timesteps x 2)
                # (If Multimodal) predictions = (Num_modes x Num_peds x Num_timesteps x 2)
                # predictions = self.get_predictions(batch_scene, batch_scene_goal, batch_split)
                
                batch_scene = torch.nan_to_num(batch_scene, 5000)
                observed = batch_scene[:self.obs_length].clone()
                ...
                #     for batch_i in batch_split[:5]:
                #         plt.clf()
                #         plt.plot(batch_scene[:self.obs_length,batch_i,0].detach().numpy(), batch_scene[:self.obs_length,batch_i,1].detach().numpy(), '-b')
                #         plt.plot(targets[batch_i,:,0], targets[batch_i,:,1], '-g')
                #         # plt.plot(predictions[batch_i,:,0].detach().numpy(), predictions[batch_i,:,1].detach().numpy(), '-r')
                #         # plt.plot(targets[:,batch_i,0], targets[:,batch_i,1], '-g')
                #         # plt.plot(outputs[self.obs_length-1:-1,batch_i,0].detach().numpy(), outputs[self.obs_length-1:-1,batch_i,1].detach().numpy(), '-r')
                #         plt.savefig('./viz/interactions/img_val_obs{}.png'.format(batch_i))

                # Positions
                # with torch.no_grad():
                #     predictions = self.model.get_predictions(observed, self.pred_length).transpose(1, 0)
                
                # Velocity
                # velocities = observed[1:]-observed[:-1]
                with torch.no_grad():
                    predictions = self.model.get_predictions(observed, pred_gt, batch_split, self.pred_length, batch_scene_goal).transpose(1, 0)
                # predictions = batch_scene[self.obs_length-1].unsqueeze(1) + torch.cumsum(predictions_vel ,1)
                ...
                #         # plt.plot(targets[:,batch_i,0], targets[:,batch_i,1], '-g')
                #         # plt.plot(outputs[self.obs_length-1:-1,batch_i,0].detach().numpy(), outputs[self.obs_length-1:-1,batch_i,1].detach().numpy(), '-r')
                #         plt.savefig('./viz/interactions/img_val{}.png'.format(batch_i))
    ...
    def train_batch(self, batch_scene, batch_scene_goal, batch_split):
        ...
        # New model FORWARD function and LOSS calculation
        # observed (Torch.Tensor): obs_length x num_tracks x 2
        # batch_scene_goal (Torch.Tensor): num_tracks x 2
        # batch_split (Torch.Tensor): batch_size + 1
        # targets: Ground-truth velocities (update appropriately)
        
        batch_scene = torch.nan_to_num(batch_scene, 5000)

        # Positions (IGNORE)
        # src_mask = generate_square_subsequent_mask(batch_scene.shape[0])
        # targets = batch_scene[self.obs_length:].clone()
        # outputs = self.model(batch_scene, src_mask)
        # loss = self.criterion(outputs[self.obs_length-1:-1], targets, batch_split) * self.batch_size

        # Velocity (IGNORE)
        # velocities = batch_scene[1:]-batch_scene[:-1]
        # src_mask = generate_square_subsequent_mask(velocities.shape[0])
        # outputs = self.model(velocities, src_mask)

        targets = batch_scene[self.obs_length:self.seq_length] - batch_scene[self.obs_length-1:self.seq_length-1]
        # Parallel Training (TEACHER FORCING) IMPORTANT !!
        # outputs = self.model(batch_scene, batch_split, batch_scene_goal)
        # loss = self.criterion(outputs[self.obs_length-2:-1], targets, batch_split) * self.batch_size

        # SEQUENTIAL Training (NO TEACHER FORCING) IMPORTANT !!
        rel_outputs, _ = self.model.forward_sequential(batch_scene[:self.obs_length], batch_scene[self.obs_length:], batch_split, batch_scene_goal)
        loss = self.criterion(rel_outputs, targets, batch_split) * self.batch_size        

        ###################### TODO (Visualization) ###############################
        # import matplotlib.pyplot as plt
        # targets1 = batch_scene[self.obs_length:].clone()
        # outputs1 = batch_scene[self.obs_length-1].unsqueeze(0)+torch.cumsum(outputs[self.obs_length-2:-1].detach(),0)
        # for batch_i in batch_split[:-1]:
        #     plt.clf()
        #     plt.plot(batch_scene[:self.obs_length,batch_i,0].detach().numpy(), batch_scene[:self.obs_length,batch_i,1].detach().numpy(), '-b')
        #     plt.plot(targets1[:,batch_i,0], targets1[:,batch_i,1], '-g')
        #     plt.plot(outputs1[:,batch_i,0].numpy(), outputs1[:,batch_i,1].numpy(), '-r')
        #     # plt.plot(targets[:,batch_i,0], targets[:,batch_i,1], '-g')
        #     # plt.plot(outputs[self.obs_length-1:-1,batch_i,0].detach().numpy(), outputs[self.obs_length-1:-1,batch_i,1].detach().numpy(), '-r')
        #     plt.savefig('./viz/img_{}.png'.format(batch_i))
        ###################### TODO ###############################
...
def main(epochs=25):
    ...
    if args.goals:
        args.output = 'OUTPUT_BLOCK/{}/new_model_goals_{}_{}.pkl'.format(args.path, args.type, args.output)
    else:
        args.output = 'OUTPUT_BLOCK/{}/new_model_{}_{}.pkl'.format(args.path, args.type, args.output)
    ...
    ###################### TODO ###############################
    # initialize new forecasting model appropriately
    model = MyNewModel(pool, args.goal_dim)
    ###################### TODO ###############################
