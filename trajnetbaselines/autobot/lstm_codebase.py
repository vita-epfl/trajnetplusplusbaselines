...
class LSTMPredictor(object):
    ...
    def __call__(self, paths, scene_goal, n_predict=12, modes=1, predict_all=True, obs_length=9, start_length=0, args=None):
        self.model.eval()
        if "real" in args.path:
            N = 12
        elif "synth" in args.path:
            N = 6
        modes = 1
        largest_num_agents = 0.0
        train_file_data = np.zeros((1, 21, N, 2))
        ego_in_return = np.zeros((1, 9, 3))
        ego_out_return = np.zeros((1, 12, 3))
        agents_in_return = np.zeros((1, 9, N-1, 3))
        agents_out_return = np.zeros((1, 12, N-1, 3))
        roads_return = np.zeros((1, 1, 1))
        agent_types_return = np.zeros((1, N, 1))

        with torch.no_grad():
            xy = trajnetplusplustools.Reader.paths_to_xy(paths)


            curr_scene_2 = drop_distant(xy, max_num_peds=N)
            curr_scene, rotation, center, = center_scene(curr_scene_2)
            ...
            if curr_scene.shape[1] < N:
                ...
                temp_curr_scene[:curr_scene.shape[0],:curr_scene.shape[1],:] = curr_scene
            ...
            batch_scene = torch.Tensor(scene)  
            batch_split = torch.Tensor(batch_split).long()

            ego_in_return[0] = ego_in
            agents_in_return[0] = agents_in[:, 1:]
            roads_return[0] = roads
            agent_types_return[0] = agent_types

            ego_in_return = torch.Tensor(ego_in_return)
            agents_in_return = torch.Tensor(agents_in_return)
            roads_return = torch.Tensor(roads_return)
            agent_types_return = torch.Tensor(agent_types_return)

            multimodal_outputs = {}
            for num_p in range(modes):
                pred_obs, mode_probs = self.model(ego_in_return, agents_in_return, roads_return, agent_types_return)
                pred_obs = pred_obs[num_p,:, 0,:,:2].numpy()
                pred_obs = inverse_scene(pred_obs, rotation, center)
                output_primary = pred_obs[-n_predict:, 0]
                output_neighs = pred_obs[-n_predict:, 1:largest_num_agents]
