"""Command line tool to train model."""
...
from trajnetbaselines.autobot.loss import PredictionLoss, L2Loss
from trajnetbaselines.autobot.lstm_codebase import LSTMPredictor, drop_distant, LSTM
from trajnetbaselines.autobot.gridbased_pooling import GridBasedPooling
from trajnetbaselines.autobot.non_gridbased_pooling import NearestNeighborMLP, HiddenStateMLPPooling, AttentionMLPPooling
from trajnetbaselines.autobot.non_gridbased_pooling import NearestNeighborLSTM, TrajectronPooling

from trajnetbaselines import __version__ as VERSION

from trajnetbaselines.autobot.utils import center_scene, random_rotation
from trajnetbaselines.autobot.data_load_utils import prepare_data

from trajnetbaselines.autobot.process_args import get_train_args
from trajnetbaselines.autobot.models.autobot_joint import AutoBotJoint
from trajnetbaselines.autobot.utilsABT.train_helpers import nll_loss_multimodes, nll_loss_multimodes_joint
...
class Trainer(object):
    def __init__(self, args, device=None, obs_length=9, pred_length=12, save_every=5, start_length=0,val_flag=True):
        ...
        # self.optimizer = torch.optim.Adam(self.autobot_model.parameters(), lr=1e-3, weight_decay=1e-4)
...
def main(epochs=25):
    ...
    args.output = 'OUTPUT_BLOCK/{}/Autobot_{}.pkl'.format(args.path, args.exp_id)
    ...
    if args.type == 'hiddenstatemlp':
        ...
        pool = GridBasedPooling(type_=args.type, hidden_dim=128,
                                cell_side=0.6, n=12, front=False,
                                out_dim=256, embedding_arch='one_layer',
                                constant=0, pretrained_pool_encoder=pretrained_pool,
                                norm=0, layer_dims=[512], latent_dim=16)
    ...
    trainer = Trainer(args, device=args.device, val_flag=val_flag)
    trainer.loop(train_scenes, val_scenes, train_goals, val_goals, args.output, epochs=args.num_epochs, start_epoch=start_epoch)
